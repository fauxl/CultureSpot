...
function chiudimini(){
	...
	for (i = 0; i < modal9.length; i++) {
	    modal9[i].style.display = "none";
	  		}
	
	for (i = 0; i < modal10.length; i++) {
	    modal10[i].style.display = "none";
	  		}
	
	for (i = 0; i < modal11.length; i++) {
	    modal11[i].style.display = "none";
	  		}
}
function chiudi(){    
...
<body>
	...
	<div id="topBar">
		...
		<div id="description">Cerca tutti i Culture Spot dei luoghi di tuo interesse!</div>
	...
	<div id="corpo">
		...
		<div id="searchBoxContainer">
			...
			<form action="home" method="post">
				<input required name="searchbar" id="searchBox"
					placeholder="Inserisci il nome della provincia..."> <i
		...
		<div id="checkContainer">
			<div id="suggest">Nascondi i risultati che non ti interessano</div>
...
<div id="contenitoreMusei" style="display: block">
				<p id="tipologiaMusei">
					...
					<a href="javascript:void(0);" onclick="myFunction()"> <i
						class="fas fa-chevron-circle-down" id="apriRisultati"></i>
					</a>
				</p>
						...
						<div id="myModal2" class="modal">
							<div id="mappa">
								...
								<div id="buttonActiveMap"
									...
									<img src=Immagini/caricamento.gif id="caricamento">
		...
		<div id="contenitoreMonumenti" style="display: block">
			<p id="tipologiaMonumenti">
				...
				<a href="javascript:void(0);" onclick="myFunction2()"><i
					class="fas fa-chevron-circle-down" id="apriRisultati"></i> </a>
			</p>
					...
					<div id="myModal5" class="modal">
						<div id="mappa">
							<div id="barraSuperiore">
								...
								<div id="buttonActiveMap"
									onmouseover="caricaMappaIndirizzo(this,'https://maps.google.com/maps?width=700&amp;height=440&amp;hl=it&amp;q=<%=bean.getIndirizzo()%>%2C%20Italy+(<%=bean.getNome()%>)&amp;ie=UTF8&amp;t=&amp;z=14&amp;iwloc=B&amp;output=embed')">
									<img
										src=Immagini/caricamento.gif
										id="caricamento"> Caricamento in corso, attendi...
								</div>
				...
				<div id="row">
					...
					<div id="df">
							...
							<div id="dettagliFilm">
								...
								<br> <span   onclick="mine9(<%=f%>)" id="inGrassetto" class="link">Clicca qui per visualizzare la trama </span>
					<div id="myminimodal1" class="minimodal">	<div id="barraSuperioreRev">
								<span id="close" class="close" onclick="chiudimini()"><i
									class="fas fa-times" style="margin: auto;"></i></span></div><div id="rev"><%=films.getTrama()%></div></div>
									
								<br> <span onclick="mine10(<%=f%>)" id="inGrassetto" class="link">Clicca qui per visualizzare dettagli del film</span>
					<div id="myminimodal2"  class="minimodal">	<div id="barraSuperioreRev">
								<span id="close" class="close" onclick="chiudimini()"><i
									class="fas fa-times" style="margin: auto;"></i></span></div><div id="rev"><%=films.getPanoramica()%></div></div>
									
								<br> <span  onclick="mine11(<%=f%>)" id="inGrassetto" class="link">Clicca qui per visualizzare una critica o curiosità </span>
					<div id="myminimodal3"  class="minimodal"><div id="barraSuperioreRev">
								<span id="close" class="close" onclick="chiudimini()"><i
									class="fas fa-times" style="margin: auto;"></i></span></div><div id="rev"><%=films.getRecensione()%></div></div>
							<%f++; %>
							</div>
							</div>
							<br>
			...
			<div class="dropdown5" id="mydrop5">
				<%
					...
					if (concerti != null && concerti.size() != 0) {
						...
						while (it.hasNext()) {
							Concerto bean = (Concerto) it.next();
				%>
				<div id="rowConc">
					<i class="fas fa-microphone-alt" id="iconaConcerti1"></i>
					<div id="dett"><%=bean.getNome()%>
					<%=bean.getData()%>,
					<%=bean.getIndirizzo()%></div>
					...
					<div id="myModal7" class="modal">
						<div id="mappa">
							...
							<div id="buttonActiveMap"
									onmouseover="caricaMappaIndirizzo(this,'https://maps.google.com/maps?width=700&amp;height=440&amp;hl=it&amp;q=<%=bean.getIndirizzo()%>%2C%20<%=bean.getProvincia()%>+(Titolo)&amp;ie=UTF8&amp;t=&amp;z=14&amp;iwloc=B&amp;output=embed')">
									<img
										src=Immagini/caricamento.gif
										id="caricamento"> Caricamento in corso, attendi...
								</div>
				...
				<div id="row">
					...
					<div id="myModal8" class="modal">
						<div id="mappa">
							...
							<div id="buttonActiveMap"
									onmouseover="caricaMappaIndirizzo(this,'https://maps.google.com/maps?width=700&amp;height=440&amp;hl=it&amp;q=<%=bean.getIndirizzo()%>%2C%20Teatro <%=bean.getNome()%>)%2C%20Italy+(Titolo)&amp;ie=UTF8&amp;t=&amp;z=14&amp;iwloc=B&amp;output=embed')">
									<img
										src=Immagini/caricamento.gif
										id="caricamento"> Caricamento in corso, attendi...
								</div>
					...
					<div id="myModal4" class="modal">
						<div id="dettagli">
							...
							<div id="dettagliFilm">
								<span id="inGrassetto">Spettacolo: </span>
								<%=specs.getSpettacolo()%>
...
var modal9 = document.querySelectorAll("[id='myminimodal1']");
var modal10 = document.querySelectorAll("[id='myminimodal2']");
var modal11 = document.querySelectorAll("[id='myminimodal3']");
...
function mine9(x) {
	  modal9[x].style.display = "block";

}
function mine10(x) {
	  modal10[x].style.display = "block";

}
function mine11(x) {
	  modal11[x].style.display = "block";
...
window.onclick = function(event) {
	...
	for (i = 0; i < modal9.length; i++) {
		if (event.target == modal9[i]) {
		    modal9[i].style.display = "none";
		  }		}
	for (i = 0; i < modal10.length; i++) {
		if (event.target == modal10[i]) {
		    modal10[i].style.display = "none";
		  }		}
	for (i = 0; i < modal11.length; i++) {
		if (event.target == modal11[i]) {
		    modal11[i].style.display = "none";
		  }		}
}
</script>
